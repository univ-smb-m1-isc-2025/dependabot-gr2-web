...
export default function RepositoryDetails({ repositoryId, onBack }: RepositoryDetailsProps) {
  // Use the defined interfaces for state
  ...
  const displayRepositoryDetails = (repository: Repository | undefined) => {
  ...
  const displayDependencies = (dependencies: Dependency[] | undefined) => {
          ...
          <tbody className="divide-y divide-gray-200 bg-gray-800 text-white">
            {dependencies.map((dep) => (
              ...
              >
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-100 w-min">
                  {(dep.old_version !== dep.new_version) 
                    ? <div className='bg-red-500 text-red-900 rounded-lg p-1 border border-red-900 border-2'>Not Up to Date</div> 
                    : <div className='bg-green-500 text-green-900 rounded-lg p-1 border border-green-900 border-2'>Up to Date</div>
                  }
                </td>
  ...
  // Fixed the useEffect dependency array
  useEffect(() => {
    fetchRepositoryDetails(repositoryId);
  }, [repositoryId, fetchRepositoryDetails]);
    ...
    <div className="flex flex-col gap-4 bg-gray-800 p-6 rounded-lg shadow-md">
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col gap-4">
          {repositoryDetails && displayRepositoryDetails(repositoryDetails.repository)}
          {repositoryDetails && displayDependencies(repositoryDetails.dependencies)}
        </div>
      )}
    </div>
  );
